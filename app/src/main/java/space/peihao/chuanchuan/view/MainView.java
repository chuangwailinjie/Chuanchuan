...
public class MainView extends BaseView{
	...
	public void initBitmap() {
		...
		f_x=(screen_width-23*screen_width/320f)/back1.getWidth();
		f_y = (screen_height - board.getHeight()-24*screen_height/480f) / back1.getHeight();
		...
		for(int i=0;i<nums.length;i++){
			for(int j=0;j<nums[0].length;j++){
				...
				Log.i("xxx",""+go.getWidth()+"   "+go.getHeight());
				al.add(go);
			}
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch(event.getAction()){
			...
			case MotionEvent.ACTION_UP:
				...
				Log.i("cFruit.size()-------：", "" + cFruits.size());
				drawSelf();
				actionup=true;
				touch=false;
				break;
		...
		if(cFruits.size()>=7){
			...
			if(count==0){
				...
				if(score<passScore){
					...
					DbHelper dbHelper = new DbHelper(mainActivity, "record", null, 1);
					SQLiteDatabase db = dbHelper.getWritableDatabase();
					long status;
					status = db.insert("tb_record", null, value);
			...
			if(actionup) {
				...
				if(cFruits.size()>=1){
					for(int i=cFruits.size()-1;i>=0;i--){
						...
						canvas.drawBitmap(go.getBitmap(),eventx-fruits.get(0).getWidth()/2.0f,(screen_height-stick_head.getHeight()-selfY)+10+go.getBitmap().getHeight()*(cFruits.size()-1-i),paint);
					}
				}
			}
